...
{
  u8* vmem;
  u8* mem_allocation_space;
  u64 num_regions; // number of allocations we can support in this page
  u64 num_regions_in_use;
  u64 bitarray_len;
  u64 unused;
};
// ensure cache line alignment, which we also do below with the subsequent bitarray, before the user data.
CompileAssert( sizeof( listelem_t<fsapage_t> ) == 64 );
struct
fsalloc_t
{
  u64 nbits;
...
Alloc( fsalloc_t* f )
{
  ...
  auto nbytes_page = sizeof( listelem_t<fsapage_t> );
  ...
  nbytes_page += nbytes_bitbuffer;
  nbytes_page += c_target_page_size;
  ...
  new_page->vmem = vmem;
  new_page->mem_allocation_space = vmem + sizeof( listelem_t<fsapage_t> ) + nbytes_bitbuffer;
