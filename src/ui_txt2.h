...
TxtLoad( txt_t& txt, file_t& file )
{
  ...
  auto nlines = Min<u32>( NLines( &txt.buf ), c_lines );
...
{
  *y = Min<u32>( LastLine( buf ), *y );
  auto line = LineFromY( buf, *y );
  *x = Min<u32>( line->len, *x );
}
...
ApplyCsUndo( txt_t& txt, cs_undo_t& undo )
{
  ...
  txt.c_x = Min<u32>( undo.c_x, CursorInlineEOL( &txt.buf, txt.c_y ) );
  txt.s_x = Min<u32>( undo.s_x, CursorInlineEOL( &txt.buf, txt.s_y ) );
...
__TxtCmd( CmdCursorGotoline )
{
  ...
  AddCsUndo( txt );
  txt.seltype = seltype_t::none;
  txt.c_y = Min<u32>( nlines, LastLine( &txt.buf ) );
  txt.c_x = Min<u32>( txt.c_x, CursorInlineEOL( &txt.buf, txt.c_y ) );
  MakeCursorVisible( txt );
}
...
MultiselectDelete( txt_t& txt, multisel_t& ms )
{
  ...
  txt.c_x = Min<u32>( ms.left_inline, CursorInlineEOL( &txt.buf, txt.c_y ) );
  txt.s_x = Min<u32>( ms.left_inline, CursorInlineEOL( &txt.buf, txt.s_y ) );
...
MultiselectRemChL( txt_t& txt, multisel_t& ms, u32 len )
{
  ...
  txt.c_x = Min<u32>( new_x, CursorInlineEOL( &txt.buf, txt.c_y ) );
  txt.s_x = Min<u32>( new_x, CursorInlineEOL( &txt.buf, txt.s_y ) );
...
MultiselectRemLn( txt_t& txt )
{
  ...
  txt.seltype = seltype_t::none;
  txt.c_y = Min<u32>( LastLine( &txt.buf ), ms.top_y );
...
RemLn( txt_t& txt )
{
  ...
  txt.c_y = Min<u32>( LastLine( &txt.buf ), txt.c_y );
...
MultiselectPaste( txt_t& txt )
{
  ...
  if( lines_to_paste.len ) {
    ...
    txt.c_x = Min<u32>( ms.left_inline + extra, CursorInlineEOL( &txt.buf, txt.c_y ) );
    txt.s_x = Min<u32>( ms.left_inline + extra, CursorInlineEOL( &txt.buf, txt.s_y ) );
...
PUBLIC	??_C@_08DLICDMBN@continue?$AA@			; `string'
	...
	// TODO: this is depending on the test .config file containing the tabs/spaces setting favoring spaces!
	// Not an idea dependency to take.
