...
txt_t
{
  ...
  array_t<cs_undo_t> redos_txtcs; // note these are locked into txt undo, which always restores previous diff state.
  array_t<cs_undo_t> undos; // used in CmdCsUndo/Redo. stores cursel state prior to each change.
  array_t<cs_undo_t> redos;
  txt_dblclick_t dblclick;
  fontlayout_t layout; // created on every EditRender call. sent to Glw for rendering.
  array_t<linespan_t> linespans; // generated by rendering, used by mouse code to map to a line position.
  array_t<charspan_t> charspans; // generated by rendering, used by mouse code to map to a char position.

  array_t<charspan_t> charspans_wrapping; // temporary array only used during rendering.
  array_t<wordspan_t> wordspans; // temporary array only used during rendering.
  array_t<idxspan_t> wrapped_lines; // temporary array only used during rendering.
  array_t<idx_t> nwords_per_wrapped_line; // temporary array only used during rendering. // TODO: buffer_t
...
SelectCopy( txt_t& txt )
{
  ...
  if( sel.sl_y != sel.sr_y  ||  sel.sl_x != sel.sr_x ) {
    string_t contents = AllocContents(
      &txt.buf,
      sel.sl_x,
      sel.sl_y,
      sel.sr_x,
...
MultiselectPaste( txt_t& txt )
{
  ...
  array32_t<slice32_t> lines_to_paste;
...
//      !txt.scroll_grabbed;
  ...
  FORLINES( &txt.buf, line, line_idx, line_start, txt.scroll_end + 1 )
    ...
    FORLEN( wrapped_line, i, txt.wrapped_lines )
      ...
      linespan->y = line_idx;
      AssertCrash( wrapped_line->len <= MAX_u32 ); // TODO: convert txt.linespans/charspans/etc. to array32_t.
      if( wrapped_line->len ) {
...
TestTxt()
{
...

  {
    ...
    CmdAddString( txt, Cast( idx_t, file ), CsLen( Str( file ) ) );
    CmdCursorFileL( txt );
...
PUBLIC	??_C@_08DLICDMBN@continue?$AA@			; `string'
...

  {
    ...
    CmdAddString( txt, Cast( idx_t, file ), CsLen( file ) );
    CmdCursorFileL( txt );
    CmdCursorR( txt );
    CmdCursorR( txt );
...

  {
    ...
    CmdAddString( txt, Cast( idx_t, file0 ), CsLen( Str( file0 ) ) );
    CmdCursorFileL( txt );
    CmdPermuteD( txt );
