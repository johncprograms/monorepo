...
eliminate stack_resizeable_cont_t and list_t usage; we can probably contiguously store everything in a single pagelist_t
...
sizeinfo_t
{
  ...
  u32 bitcount;
  u32 bitalign;
  stack_resizeable_cont_t<sizeinfo_t> fields;
  stack_resizeable_cont_t<u32> field_bitoffsets;
};
Inl void
...
node_num_t
{
  ...
  f64 value_f64;
  f32 value_f32;
  listwalloc_t<ptrtype_t, allocator_pagelist_t, allocation_pagelist_t> ptrtypes; // TODO: TypeE1 determines what the E0's ptrtypes is, but doesn't change this.
  sizeinfo_t sizeinfo;
};
Inl void
Init( node_num_t* node, pagelist_t* mem )
{
  Typezero( node );
  node->nodetype = nodetype_t::num;
  node->ptrtypes.alloc = allocator_pagelist_t{ mem };
}
...
node_str_t
{
  ...
  token_t* tkn;
  symbol_t literal;
  listwalloc_t<ptrtype_t, allocator_pagelist_t, allocation_pagelist_t> ptrtypes;
  sizeinfo_t sizeinfo;
};
Inl void
Init( node_str_t* node, pagelist_t* mem )
{
  Typezero( node );
...
Inl void
Init( node_chr_t* node, pagelist_t* mem )
{
  Typezero( node );
...
Inl void
Init( node_binop_t* node, pagelist_t* mem )
{
  Typezero( node );
...
PrecedenceValue( binoptype_t type )
{
  switch( type ) {
    ...
    case binoptype_t::xor_: { return 99999 /* TODO */; } break;

    case binoptype_t::shl:
    case binoptype_t::shr: { return 99999 /* TODO */; } break;
...
node_funccall_t
{
  ...
  token_t* tkn;
  node_ident_t* ident;
  stack_resizeable_cont_t<node_e5_t*> e5_args;
  listwalloc_t<ptrtype_t> ptrtypes;
  sizeinfo_t sizeinfo;
};
Inl void
Init( node_funccall_t* node, pagelist_t* mem )
{
  Typezero( node );
...
node_identdot_t
{
  nodetype_t nodetype;
  token_t* tkn;
  stack_resizeable_cont_t<node_ident_t*> idents;
  listwalloc_t<ptrtype_t> ptrtypes;
  sizeinfo_t sizeinfo;
};
Inl void
Init( node_identdot_t* node, pagelist_t* mem )
{
  Typezero( node );
...
Inl void
Init( node_unop_t* node, pagelist_t* mem )
{
  Typezero( node );
...
node_e0_t
{
  ...
  union {
    ...
    stack_resizeable_cont_t<node_e5_t*> e5s; // tuple; unfinished for now.
    node_unop_t* unop;
    node_num_t* num;
    node_str_t* str;
    node_chr_t* chr;
...
Inl void
Init( node_e0_t* node, pagelist_t* mem )
{
  Typezero( node );
...
node_funcdeclordefn_t
{
  ...
  token_t* tkn;
  node_ident_t* ident;
  stack_resizeable_cont_t<node_decl_t*> decls;
  node_type_t* type;
  node_scope_t* scope;
};
Inl void
...
Inl void
Init( node_binassign_t* node, pagelist_t* mem )
{
  Typezero( node );
...
Inl void
Init( node_ret_t* node, pagelist_t* mem )
{
  Typezero( node );
...
Inl void
Init( node_while_t* node, pagelist_t* mem )
{
  Typezero( node );
...
node_ifchain_t
{
  ...
  stack_resizeable_cont_t<node_e5_t*> elif_e5s;
  stack_resizeable_cont_t<node_scope_t*> elif_scopes;
  node_scope_t* else_scope;
  listwalloc_t<ptrtype_t> ptrtypes;
  sizeinfo_t sizeinfo;
};
Inl void
Init( node_ifchain_t* node, pagelist_t* mem )
{
  Typezero( node );
...
node_switch_t
{
  ...
  node_e5_t* e5;
  node_scope_t* scope;
  stack_resizeable_cont_t<node_case_t*> cases; // TODO:
...
Inl void
Init( node_defer_t* node, pagelist_t* mem )
{
  Typezero( node );
...
Inl void
Init( node_statement_t* node, pagelist_t* mem )
{
  Typezero( node );
...
coretype_func_t
{
  coretypetype_t type;
  symbol_t name;
  stack_resizeable_cont_t<ptrtype_t> args;
  bool has_ret;
  ptrtype_t ret;
};
...
coretype_struct_t
{
  coretypetype_t type;
  symbol_t name;
  stack_resizeable_cont_t<ptrtype_t> fields;
  stack_resizeable_cont_t<symbol_t> fieldnames;
};
...
code_funcdefnstart_t
{
  ...
  stack_resizeable_cont_t<sizeinfo_t> args; // TODO: how does the fnbody's codegen know how to address these args?
  bool has_retval;
};
...
{
  codetype_t type;
  stack_resizeable_cont_t<code_var_t*> var_args; // PERF: terrible.
...
Inl void Init( code_funccall_t* code )
{
  ...
  // stack_resizeable_cont_t is set up elsewhere
}
...
node_scope_t
{
  nodetype_t nodetype;
  token_t* tkn;
  stack_resizeable_cont_t<node_statement_t*> statements;
...
Inl void
Init( node_scope_t* node, pagelist_t* mem )
{
  Typezero( node );
...
Inl void
PrintAstNode( ast_t& ast, slice_t& src, stack_resizeable_cont_t<u8>& dst, void* elem )
{
  ...
  switch( type ) {
    ...
    case nodetype_t::unop: {
      ...
      auto unoptype_len = CstrLength( unoptype );
...
//      auto binoptype_len = CstrLength( binoptype );
...
//      node_break_t& node = *Cast( node_break_t*, elem );
    ...
    case nodetype_t::binassign: {
      ...
      auto binassigntype_len = CstrLength( binassigntype );
...
Inl void
PrintAst( ast_t& ast, slice_t& src, stack_resizeable_cont_t<u8>& dst )
{
