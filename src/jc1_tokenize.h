...
tlog_t
{
  idx_t nerrs;
  idx_t nwarns;
  stack_resizeable_cont_t<u8> errs;
  stack_resizeable_cont_t<u8> warns;
};
...
OutputFileAndLine(
  slice_t& filename,
  stack_resizeable_cont_t<u8>& file,
  token_t* tkn
  )
{
  stack_nonresizeable_stack_t<u8, 64> lineno;
  CsFrom_u64( lineno.mem, Capacity( lineno ), &lineno.len, Cast( u64, tkn->lineno ) );
  stack_nonresizeable_stack_t<u8, 64> inlinech;
  ...
  )
{
  ...
  u8* eol = Cast( u8*, MemScanR( bol, src->file.len - ( bol - src->file.mem ), "\n", 1 ) );
  idx_t linelen = CstrLength( bol, eol );
  ...
  )
{
  ...
  auto errstr_len = CstrLength( Cast( u8*, errstr ) );
...
static void
PostProcessTokens( stack_resizeable_cont_t<token_t>& tokens, src_t* src, tlog_t* log )
{
  ...
  )
{
  ...
  // this assumes AsciiIsNumber( curr[0] ), which is checked by the caller.
  idx_t offset = 1;
  idx_t ndots = 0;
  bool last_was_dot = 0;
...
void
Tokenize( stack_resizeable_cont_t<token_t>& tokens, src_t* src, tlog_t* log )
{
  idx_t last_bol = 0;
  idx_t line_count = 1;
  ...
  while( pos < src->file.len ) {
    ...
    //if( ( curr[0] == '-' )  |  AsciiIsNumber( curr[0] ) ) {
    //  if( curr[0] == '-' ) {
    //    if( pos + 1 < src->file.len ) {
    //      if( AsciiIsNumber( curr[1] ) ) {
    //        idx_t offset;
    //        TokenizeNumeral( src, tokens, pos, curr, tkn, &offset, log );
    //        if( log->nerrs ) return;
    //        pos += offset;
...
Inl void
PrintTokens( stack_resizeable_cont_t<token_t>& tokens, slice_t& src )
{
  ForLen( i, tokens ) {
