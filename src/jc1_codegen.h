...
Templ Inl T*
_AddCode( stack_resizeable_cont_t<code_t>& code )
{
...
GEN1( node_funccall_t, GenFunccall )
{
  ...
  stack_resizeable_cont_t<code_var_t*> var_args;
...
GEN0( node_scope_t, GenScope )
{
  ...
  stack_resizeable_cont_t<code_t> deferred_code;
...
Inl void
PrintGen( stack_resizeable_cont_t<code_t>& code )
{
  ForLen( i, code ) {
    auto c = code.mem + i;
    switch( c->type ) {
...
Inl void
Execute( stack_resizeable_cont_t<code_t>& code )
{
  if( !code.len ) {
    return;
  }
