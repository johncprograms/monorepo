  ...
  )
{
  ...
  if( !rows[ num_carries ] ) {
    rows[ num_carries ] = AddPlist( *mem, int, _SIZEOF_IDX_T, carry_row_len );
  }
  ...
  )
{
  ...
  auto num_rows = zipset_t::c_num_rows - _lzcnt_idx_t( N );
  For( i, 0, num_rows ) {
    auto row = rows[ i ];
    auto row_len = 1u << i;
  ...
  )
{
  ...
  For( i, 0, num_rows ) {
    ...
    if( !( N & row_len ) ) {
      if( row ) {
        // TODO: free( row ) for non-plist allocators.
        rows[ i ] = 0;
      }
    }
  }
