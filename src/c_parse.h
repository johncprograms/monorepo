  ...
  )
{
  ...
  idx_t foundtype_len = CsLen( foundtype );
  ...
  For( i, 0, expected_len ) {
    ...
    idx_t exptype_len = CsLen( exptype );
  ...
  )
{
  ...
  if( *pos >= tokens->len ) {
    ...
    idx_t str_type_len = CsLen( str_type );
  ...
  if( tkn->type != type ) {
    ...
    idx_t str_foundtype_len = CsLen( str_foundtype );
...
PARSE( node_num_t, ParseNum )
{
  ...
  auto node = AddPlist( *context->mem, node_num_t, _SIZEOF_IDX_T, 1 );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_str_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_chr_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_ident_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_e1_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_e2_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_e3_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_e4_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_e5_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_funccall_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_identdot_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_unop_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_e0_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
  switch( node->tkn->type ) {
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_type_t, _SIZEOF_IDX_T, 1 );
  Init( node );
  node->tkn = ExpectToken( context, *pos );
  node->ptrlevel = 0;
  node->ident = 0;
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_decl_t, _SIZEOF_IDX_T, 1 );
  Init( node );
  node->tkn = ExpectToken( context, *pos );
  node->ident = ParseIdent( ast, context, pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_funcdeclordefn_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_datadecl_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_binassign_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
  node->e0_l = ParseE0( ast, context, pos );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_ret_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_continue_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_break_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_while_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_ifchain_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_switch_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_case_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_default_t, _SIZEOF_IDX_T, 1 );
  Init( node );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_defer_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_statement_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  node->tkn = ExpectToken( context, *pos );
  switch( node->tkn->type ) {
    case tokentype_t::fn: {
    ...
    case tokentype_t::ident: {
      ...
      if( tkn->type == tokentype_t::paren_l ) {
        ...
        if( ArrayContains( AL( tkns_binassign ), &tkn->type ) ) {
...
{
  if( context->log->nerrs ) return 0;
  auto node = AddPlist( *context->mem, node_scope_t, _SIZEOF_IDX_T, 1 );
  Init( node, context->mem );
  ...
  )
{
  ...
  auto toplevel = AddPlist( *context->mem, node_scope_t, _SIZEOF_IDX_T, 1 );
