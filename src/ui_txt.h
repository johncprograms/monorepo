...
txt_t
{
  ...
  stack_resizeable_cont_t<cs_undo_t> redos_txtcs; // no need to update as concurrents, since these are locked into txt undo, which always restores previous diff state.
  stack_resizeable_cont_t<cs_undo_absolute_t> undos; // used in CmdCsUndo/Redo. stores cursel state prior to each change.
  stack_resizeable_cont_t<cs_undo_absolute_t> redos; // we get away with not updating these as concurrents, since they're absolute indices.
  ...
  stack_resizeable_cont_t<linerect_t> linerects; // generated by rendering, used by mouse code to map to a line position.
  ...
  Forever {
    ...
    if( line_len ) {
      ...
      if( draw_cursorwordmatch  &&  word.len ) {
        ...
        stack_resizeable_cont_t<txt_wordspan_t> spans;
        Alloc( spans, 64 );
        {
          txt_wordspan_t span;
          span.l = line_start;
